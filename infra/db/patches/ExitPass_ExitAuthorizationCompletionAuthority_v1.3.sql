...
SET tariff_snapshot_id = attempt.tariff_snapshot_id,
    ...
    completion_authority_reference_id = exit_auth.payment_confirmation_id
FROM core.payment_attempts AS attempt
WHERE attempt.payment_attempt_id = exit_auth.payment_attempt_id
  AND (exit_auth.tariff_snapshot_id IS NULL OR exit_auth.completion_basis IS NULL OR
       exit_auth.completion_authority_reference_id IS NULL);
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'fk_exit_authorizations__policy_version_id'
          AND conrelid = 'core.exit_authorizations'::regclass
    ) THEN
        ALTER TABLE core.exit_authorizations
            ADD CONSTRAINT fk_exit_authorizations__policy_version_id
...
BEGIN
    ...
    IF NEW.completion_authority_reference_id IS NULL AND
       NEW.completion_basis = 'PAYMENT_FINALITY' THEN
        NEW.completion_authority_reference_id := NEW.payment_confirmation_id;
    END IF;

    IF NEW.completion_basis = 'PAYMENT_FINALITY' THEN
        IF NEW.payment_attempt_id IS NULL OR NEW.payment_confirmation_id IS NULL OR
           NEW.completion_authority_reference_id IS DISTINCT FROM NEW.payment_confirmation_id OR
           ...
           NEW.applied_policy_reference_id IS NOT NULL OR
           NEW.statutory_discount_policy_version_id IS NOT NULL THEN
    ...
    ELSIF NEW.completion_basis = 'ZERO_PAYABLE_STATUTORY_FINALITY' THEN
        IF NEW.payment_attempt_id IS NOT NULL OR NEW.payment_confirmation_id IS NOT NULL OR
           ...
           NEW.completion_authority_reference_id IS DISTINCT FROM
               NEW.statutory_discount_payable_basis_application_command_id OR
           ...
           (NEW.applied_policy_reference_id IS NULL AND
            NEW.statutory_discount_policy_version_id IS NULL) OR
           (NEW.applied_policy_reference_id IS NOT NULL AND
            NEW.statutory_discount_policy_version_id IS NOT NULL) THEN
...
ALTER TABLE core.exit_authorizations
    ...
    CHECK (
        (
            ...
            completion_authority_reference_id = payment_confirmation_id AND
            ...
            applied_policy_reference_id IS NULL AND
        ...
        (
            ...
            completion_authority_reference_id = statutory_discount_payable_basis_application_command_id AND
...
ALTER TABLE core.exit_authorizations
    DROP CONSTRAINT IF EXISTS ck_exit_authorizations__consumed_state;

ALTER TABLE core.exit_authorizations
    ADD CONSTRAINT ck_exit_authorizations__consumed_state CHECK (
...
COMMENT ON COLUMN core.exit_authorizations.completion_authority_reference_id IS
    'Durable authority identifier for the selected completion basis.';
...
COMMENT ON COLUMN core.exit_authorizations.applied_policy_reference_id IS
    'Legacy applied policy reference backing zero-payable completion.';
COMMENT ON COLUMN core.exit_authorizations.statutory_discount_policy_version_id IS
    'Canonical immutable policy version backing zero-payable completion; legacy policy references remain separate.';
COMMENT ON COLUMN core.exit_authorizations.consumed_at IS
    'Timestamp when the authorization was consumed through the canonical completion path.';
