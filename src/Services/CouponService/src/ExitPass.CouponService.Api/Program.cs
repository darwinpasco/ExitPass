...
builder.Services
    .AddHealthChecks()
    .AddCheck("self", () => HealthCheckResult.Healthy("Coupon Service is alive."));
...
if (app.Environment.IsDevelopment() || app.Environment.EnvironmentName == "SecureDevelopment")
{
    ...
    {
        options.SwaggerEndpoint("/swagger/v1/swagger.json", "ExitPass Coupon Service API v1");
    });
}
...
{
    service = "ExitPass Coupon Service API",
    status = "running"
}));
app.Run();
