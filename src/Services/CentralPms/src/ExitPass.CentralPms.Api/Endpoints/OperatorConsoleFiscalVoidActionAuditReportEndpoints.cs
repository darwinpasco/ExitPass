...
/// <summary>
/// Operator Console read-only fiscal void action audit review endpoint.
/// </summary>
public static class OperatorConsoleFiscalVoidActionAuditReportEndpoints
{
    private const string AuditReadPolicy = "FiscalVoidActionAuditReview";
    private const string WorkflowCode = OperatorConsoleActionCodes.FiscalIssuanceStatusVisibilityWorkflow;
    private static readonly ActivitySource ActivitySource = new("ExitPass.CentralPms.Api.OperatorConsoleFiscalVoidActionAuditReport");

    /// <summary>
    /// Maps Operator Console fiscal void action audit review endpoints.
    /// </summary>
    public static IEndpointRouteBuilder MapOperatorConsoleFiscalVoidActionAuditReportEndpoints(this IEndpointRouteBuilder app)
    {
        var group = app.MapGroup("/v1/ops/operator-console")
            .WithTags("OperatorConsole");

        group.MapGet("/audit/fiscal-void-actions", ListAsync)
            .WithName("ListOperatorConsoleFiscalVoidActionAuditReport")
            .WithTags("OperatorConsole")
            .WithMetadata(new ReconciliationPolicyMetadata(AuditReadPolicy))
            .Produces<OperatorConsoleFiscalVoidActionAuditReportResponse>(StatusCodes.Status200OK)
            .Produces<ErrorResponse>(StatusCodes.Status400BadRequest)
            .Produces<ErrorResponse>(StatusCodes.Status401Unauthorized)
            .Produces<ErrorResponse>(StatusCodes.Status403Forbidden)
            .Produces<ErrorResponse>(StatusCodes.Status500InternalServerError)
            .WithSummary("List Operator Console fiscal void action audit review rows")
            .WithDescription("Returns read-only safe metadata for VOID_FISCAL_DOCUMENT action-log entries. This endpoint does not return raw payloads and does not call POS Server or mutate fiscal, payment, exit, gate, refund, reversal, retry, readback, writeback, or document-rendering state.");
    ...
    private static async Task<IResult> ListAsync(
        ...
        IOperatorConsoleFiscalVoidActionAuditReportService service,
        ...
        ILoggerFactory loggerFactory)
    {
        ...
        using var activity = ActivitySource.StartActivity("HTTP ListOperatorConsoleFiscalVoidActionAuditReport", ActivityKind.Server);
        var logger = loggerFactory.CreateLogger("ExitPass.CentralPms.Api.OperatorConsoleFiscalVoidActionAuditReportEndpoints");
        ...
        try
        {
            ...
            if (!access.Allowed)
            {
                ...
                return Results.Json(
                    BuildError(
                        "OPERATOR_CONSOLE_FISCAL_VOID_ACTION_AUDIT_REVIEW_ACCESS_DENIED",
                        "Access denied for the fiscal void action audit review.",
                        effectiveCorrelationId),
                    statusCode: StatusCodes.Status403Forbidden);
            }

            var result = await service.ListAsync(
                new OperatorConsoleFiscalVoidActionAuditReportQuery(
                    from,
                    to,
        ...
        catch (ArgumentException ex)
        {
            ...
            return Results.BadRequest(BuildError(
                "INVALID_OPERATOR_CONSOLE_FISCAL_VOID_ACTION_AUDIT_REVIEW_REQUEST",
                ex.Message,
                effectiveCorrelationId));
        }
        catch (Exception ex)
        {
            ...
            logger.LogError(ex, "Operator Console fiscal void action audit review read failed.");
            return Results.Json(
                BuildError(
                    "OPERATOR_CONSOLE_FISCAL_VOID_ACTION_AUDIT_REVIEW_READ_FAILED",
                    "The Operator Console fiscal void action audit review could not be loaded.",
                    effectiveCorrelationId),
        ...
        HttpRequest httpRequest)
    {
        var evaluation = await accessEvaluationService.EvaluateAsync(
            new OperatorConsoleAccessEvaluationCommand(
                ...
                $"operator-console-fiscal-void-action-audit-report-{identity.CorrelationId:N}",
    ...
    private static OperatorConsoleFiscalVoidActionAuditReportResponse ToContract(
        OperatorConsoleFiscalVoidActionAuditReportResult result) =>
        new(
            result.Items.Select(item => new OperatorConsoleFiscalVoidActionAuditReportItem(
                ...
                item.PosServerFiscalDocumentId,
                item.ReasonCode,
                item.ReasonText,
                item.CorrelationId,
                item.OperatorActionRequestId,
                item.PosServerResultClassification,
                ...
                item.FiscalSequenceChangedByCentralPms)).ToArray(),
            result.TotalCount,
            result.Limit,
            result.Offset,
