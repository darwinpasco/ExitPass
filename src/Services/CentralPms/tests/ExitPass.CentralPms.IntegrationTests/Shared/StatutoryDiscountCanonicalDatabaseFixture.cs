        ...
        string TaskOwnedRunId)
    {
        public static StatutoryDiscountCanonicalDatabaseFixtureOptions Load(bool validateFiles = true)
        {
            ...
            var applicationSchemaSources = new[]
            {
                ...
                new ApplicationSchemaSource(
                    "zero-payable ExitAuthorization issuance",
                    Path.Combine(patchRoot, "infra", "db", "patches", "ExitPass_Core_IssueExitAuthorization_v1.3.sql"),
                    Path.Combine(patchRoot, "infra", "db", "patches", "validation", "Validate_CoreIssueExitAuthorization_v1.3.sql"))
            };
