<#
.SYNOPSIS
Runs the aligned-DB discounted payment/fiscal ExitAuthorization readiness proof.
...
that issues a discounted Sales Invoice through local POS Server before proving
ExitAuthorization readiness/issuance without gate consumption.
#>
...
param(
    ...
    [string] $DbName = "centralpms_aligned_discount_exit_authorization_runtime_local",
...
Write-Host "ExitPass aligned-DB discounted payment/fiscal ExitAuthorization readiness proof"
...
if (-not $SkipRuntimeProof) {
    ...
    Write-Host "Running Central PMS opt-in discounted payment/fiscal ExitAuthorization readiness proof."
    dotnet test $testProjectPath `
        --no-restore `
        --filter "FullyQualifiedName~LocalRuntime_WhenEnabled_DiscountedPaymentAndFiscalIssuanceAreReadyForExitAuthorization" `
        -m:1 `
        /p:UseSharedCompilation=false

    if ($LASTEXITCODE -ne 0) {
        throw "Central PMS discounted payment/fiscal ExitAuthorization readiness proof failed with exit code $LASTEXITCODE."
    }
}

Write-Host ""
Write-Host "Aligned DB discounted payment/fiscal ExitAuthorization readiness proof PASSED."
