-- scripts/dev-data/webpay-20260521-seed.sql
-- ExitPass v1.2 WebPay runtime test data
-- Batch: WEBPAY-20260521
-- Total records: 50 parking sessions
--
-- Validity:
-- Philippine time: 2026-05-21 00:00:00 +08:00 to 2026-05-21 23:59:59 +08:00
-- UTC equivalent:  2026-05-20 16:00:00Z to 2026-05-21 15:59:59Z
--
...
--
-- Cleanup scope:
-- Only WEBPAY-20260521 records are removed/replaced.

BEGIN;

DO $$
DECLARE
    v_valid_from timestamptz := TIMESTAMPTZ '2026-05-20 16:00:00+00';
    v_valid_to   timestamptz := TIMESTAMPTZ '2026-05-21 15:59:59+00';
...
BEGIN
    ...
    VALUES (
        'WEBPAY_20260521_TEST_SEEDER',
        'WebPay 2026-05-21 Test Seeder',
        'INTERNAL_SERVICE',
        'ACTIVE',
        'ExitPass Dev Data',
        'NONE',
    ...
    VALUES (
        'WEBPAY_20260521_GROUP',
        'WebPay Test Site Group 2026-05-21',
        'Property',
        'Temporary WebPay runtime test site group for 2026-05-21 only.',
        'Pro Parking Group',
        'Asia/Manila',
        'PHP',
        'ACTIVE',
    ...
    VALUES (
        v_site_group_id,
        'WEBPAY_20260521_SITE',
        'WebPay Test Site 2026-05-21',
        'Temporary WebPay runtime test site for 2026-05-21 only.',
        'MALL_PARKING',
        'Asia/Manila',
        'WebPay Test Address',
        'Manila',
    ...
    VALUES (
        'WEBPAY_20260521_MOCK_PMS',
        'WebPay 2026-05-21 Mock Vendor PMS',
        'VENDOR_PMS',
        'ACTIVE',
        'DEV',
        'mock://webpay-20260521',
        'v1',
        'ExitPass Dev',
        v_valid_from,
        v_valid_to,
    ...
    VALUES (
        'PAYMONGO_QRPH_WEBPAY_20260521',
        'PayMongo QRPh WebPay Test Rail 2026-05-21',
        'PAYMONGO',
        'QRPH',
        'PHP',
        'ACTIVE',
    ...
    WHERE ea.parking_session_id = ps.parking_session_id
      AND ps.vendor_session_ref LIKE 'WEBPAY-20260521%';
    ...
    WHERE sii.identifier_masked LIKE 'WEBPAY-20260521%';
    ...
    FOR v_scenario, v_ticket, v_seq IN
        ...
        FROM (
            SELECT
                ...
                'WEBPAY-20260521-FRESH-' || LPAD(n::text, 3, '0') AS ticket_number,
            ...
            SELECT
                ...
                'WEBPAY-20260521-RESUME-' || LPAD(n::text, 3, '0') AS ticket_number,
                n AS scenario_sequence
            ...
            SELECT
                ...
                'WEBPAY-20260521-ORPHAN-NOSESSION-' || LPAD(n::text, 3, '0') AS ticket_number,
                n AS scenario_sequence
            FROM generate_series(1, 10) AS n
            ...
            SELECT
                ...
                'WEBPAY-20260521-ORPHAN-NOURL-' || LPAD(n::text, 3, '0') AS ticket_number,
                n AS scenario_sequence
    ...
    LOOP
        ...
        VALUES (
            ...
            v_ticket,
            TIMESTAMPTZ '2026-05-21 02:00:00+08' + (v_seq || ' minutes')::interval,
            'ACTIVE',
            'ACTIVE',
            gen_random_uuid(),
            v_service_identity_id,
        ...
        VALUES (
            ...
            v_amount,
            'ACTIVE',
            TIMESTAMPTZ '2026-05-21 08:00:00+08',
            v_valid_to,
            gen_random_uuid(),
            v_service_identity_id,
            v_service_identity_id
        ...
        IF v_scenario IN ('RESUME', 'ORPHAN_NOSESSION', 'ORPHAN_NOURL') THEN
            ...
            VALUES (
                ...
                'WEBPAY-20260521-' || v_scenario || '-' || LPAD(v_seq::text, 3, '0'),
                v_payment_rail_id,
                'PHP',
                v_amount,
                'REQUESTED',
                TIMESTAMPTZ '2026-05-21 08:10:00+08',
                v_valid_to,
                gen_random_uuid(),
                v_service_identity_id,
                v_service_identity_id
            ...
            IF v_scenario = 'RESUME' THEN
                ...
                VALUES (
                    ...
                    'WEBPAY-20260521-PROVIDER-RESUME-' || LPAD(v_seq::text, 3, '0'),
                    'ACTIVE',
                    'PHP',
                    v_amount,
                    'https://checkout.test.paymongo.local/webpay-20260521/resume/' || LPAD(v_seq::text, 3, '0'),
                    'QRPH-TEST-PAYLOAD-WEBPAY-20260521-RESUME-' || LPAD(v_seq::text, 3, '0'),
                    v_valid_to,
                    TIMESTAMPTZ '2026-05-21 08:11:00+08',
                    v_valid_to,
                    'WEBPAY-20260521-SEEDED-RESUME',
                    gen_random_uuid(),
                    v_service_identity_id,
                    v_service_identity_id
                );
            ELSIF v_scenario = 'ORPHAN_NOURL' THEN
                ...
                VALUES (
                    ...
                    'WEBPAY-20260521-PROVIDER-ORPHAN-NOURL-' || LPAD(v_seq::text, 3, '0'),
                    'ACTIVE',
                    'PHP',
                    v_amount,
                    NULL,
                    ...
                    TIMESTAMPTZ '2026-05-21 08:11:00+08',
                    v_valid_to,
                    'WEBPAY-20260521-SEEDED-ORPHAN-NOURL',
                    gen_random_uuid(),
                    v_service_identity_id,
                    v_service_identity_id
                );
    ...
    RAISE NOTICE 'WEBPAY-20260521 seed completed.';
