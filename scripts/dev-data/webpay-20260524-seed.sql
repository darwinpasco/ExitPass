...
-- Source seed date: 2026-05-23
-- Target seed date: 2026-05-24
...
-- scripts/dev-data/webpay-20260524-seed.sql
-- ExitPass v1.2 WebPay runtime test data
-- Batch: WEBPAY-20260524
-- Total records: 50 parking sessions
--
-- Validity:
-- Philippine time: 2026-05-24 00:00:00 +08:00 to 2026-05-24 23:59:59 +08:00
-- UTC equivalent:  2026-05-22 16:00:00Z to 2026-05-24 15:59:59Z
--
...
--
-- Cleanup scope:
-- Only WEBPAY-20260524 records are removed/replaced.
...
DECLARE
    ...
    v_valid_to   timestamptz := TIMESTAMPTZ '2026-05-24 15:59:59+00';
...
BEGIN
    ...
    VALUES (
        'WEBPAY_20260524_TEST_SEEDER',
        'WebPay 2026-05-24 Test Seeder',
        'INTERNAL_SERVICE',
        'ACTIVE',
        'ExitPass Dev Data',
        'NONE',
    ...
    VALUES (
        'WEBPAY_20260524_GROUP',
        'WebPay Test Site Group 2026-05-24',
        'Property',
        'Temporary WebPay runtime test site group for 2026-05-24 only.',
        'Pro Parking Group',
        'Asia/Manila',
        'PHP',
        'ACTIVE',
    ...
    VALUES (
        v_site_group_id,
        'WEBPAY_20260524_SITE',
        'WebPay Test Site 2026-05-24',
        'Temporary WebPay runtime test site for 2026-05-24 only.',
        'MALL_PARKING',
        'Asia/Manila',
        'WebPay Test Address',
        'Manila',
    ...
    VALUES (
        'WEBPAY_20260524_MOCK_PMS',
        'WebPay 2026-05-24 Mock Vendor PMS',
        'VENDOR_PMS',
        'ACTIVE',
        'DEV',
        'mock://webpay-20260524',
        'v1',
        'ExitPass Dev',
        v_valid_from,
        v_valid_to,
    ...
    VALUES (
        'PAYMONGO_QRPH_WEBPAY_20260524',
        'PayMongo QRPh WebPay Test Rail 2026-05-24',
        'PAYMONGO',
        'QRPH',
        'PHP',
        'ACTIVE',
    ...
    WHERE sii.identifier_masked LIKE 'WEBPAY-20260524%';
    ...
    FOR v_scenario, v_ticket, v_seq IN
        ...
        FROM (
            SELECT
                ...
                'WEBPAY-20260524-FRESH-' || LPAD(n::text, 3, '0') AS ticket_number,
            ...
            SELECT
                ...
                'WEBPAY-20260524-RESUME-' || LPAD(n::text, 3, '0') AS ticket_number,
                n AS scenario_sequence
            ...
            SELECT
                ...
                'WEBPAY-20260524-ORPHAN-NOSESSION-' || LPAD(n::text, 3, '0') AS ticket_number,
                n AS scenario_sequence
            FROM generate_series(1, 10) AS n
            ...
            SELECT
                ...
                'WEBPAY-20260524-ORPHAN-NOURL-' || LPAD(n::text, 3, '0') AS ticket_number,
                n AS scenario_sequence
    ...
    LOOP
        ...
        VALUES (
            ...
            v_ticket,
            TIMESTAMPTZ '2026-05-24 02:00:00+08' + (v_seq || ' minutes')::interval,
            'ACTIVE',
            'ACTIVE',
            gen_random_uuid(),
            v_service_identity_id,
        ...
        VALUES (
            ...
            v_amount,
            'ACTIVE',
            TIMESTAMPTZ '2026-05-24 08:00:00+08',
            v_valid_to,
            gen_random_uuid(),
            v_service_identity_id,
            v_service_identity_id
        ...
        IF v_scenario IN ('RESUME', 'ORPHAN_NOSESSION', 'ORPHAN_NOURL') THEN
            ...
            VALUES (
                ...
                'WEBPAY-20260524-' || v_scenario || '-' || LPAD(v_seq::text, 3, '0'),
                v_payment_rail_id,
                'PHP',
                v_amount,
                'REQUESTED',
                TIMESTAMPTZ '2026-05-24 08:10:00+08',
                v_valid_to,
                gen_random_uuid(),
                v_service_identity_id,
                v_service_identity_id
            ...
            IF v_scenario = 'RESUME' THEN
                ...
                VALUES (
                    ...
                    'WEBPAY-20260524-PROVIDER-RESUME-' || LPAD(v_seq::text, 3, '0'),
                    'ACTIVE',
                    'PHP',
                    v_amount,
                    'https://checkout.test.paymongo.local/webpay-20260524/resume/' || LPAD(v_seq::text, 3, '0'),
                    'QRPH-TEST-PAYLOAD-WEBPAY-20260524-RESUME-' || LPAD(v_seq::text, 3, '0'),
                    v_valid_to,
                    TIMESTAMPTZ '2026-05-24 08:11:00+08',
                    v_valid_to,
                    'WEBPAY-20260524-SEEDED-RESUME',
                    gen_random_uuid(),
                    v_service_identity_id,
                    v_service_identity_id
                );
            ELSIF v_scenario = 'ORPHAN_NOURL' THEN
                ...
                VALUES (
                    ...
                    'WEBPAY-20260524-PROVIDER-ORPHAN-NOURL-' || LPAD(v_seq::text, 3, '0'),
                    'ACTIVE',
                    'PHP',
                    v_amount,
                    NULL,
                    ...
                    TIMESTAMPTZ '2026-05-24 08:11:00+08',
                    v_valid_to,
                    'WEBPAY-20260524-SEEDED-ORPHAN-NOURL',
                    gen_random_uuid(),
                    v_service_identity_id,
                    v_service_identity_id
                );
    ...
    RAISE NOTICE 'WEBPAY-20260524 seed completed.';
...
SELECT
    CASE
        WHEN ps.vendor_session_ref LIKE 'WEBPAY-20260524-FRESH-%' THEN 'FRESH'
        WHEN ps.vendor_session_ref LIKE 'WEBPAY-20260524-RESUME-%' THEN 'RESUME'
        WHEN ps.vendor_session_ref LIKE 'WEBPAY-20260524-ORPHAN-NOSESSION-%' THEN 'ORPHAN_NOSESSION'
        WHEN ps.vendor_session_ref LIKE 'WEBPAY-20260524-ORPHAN-NOURL-%' THEN 'ORPHAN_NOURL'
        ELSE 'UNKNOWN'
    END AS scenario,
...
WHERE snapshot_status = 'ACTIVE'
  AND parking_session_id IN (
      ...
      WHERE vendor_session_ref LIKE 'WEBPAY-20260524%'
  )
GROUP BY parking_session_id
HAVING COUNT(*) > 1;

SELECT
    COUNT(*) AS active_tariff_snapshots_outside_may_24_utc
