# ExitPass Central PMS POS Server Controlled UAT Execution Rerun 3 Result v1.0

## 1. Execution Timestamp

| Field | Value |
| --- | --- |
| Execution started | `2026-07-09T16:57:41.5733152+08:00` |
| Execution completed | `2026-07-09T16:57:54.7051034+08:00` |
| Prior rerun result | `docs/v1.3/central-pms/runbooks/ExitPass_Central_PMS_POS_Server_Controlled_UAT_Execution_Rerun_2_Result_v1.0.md` |
| Final result | `failed` |

The approved rerun-3 request was submitted exactly once. Central PMS again returned the controlled safe failure `controlled_uat_fixture_prepare_failed`. The failure occurred before fiscal issuance reference preparation, before diagnostic invocation, and before any POS Server fiscal document call.
...
| Build evidence | `D:\ExitPass-UAT-Evidence\CPS-POS-UAT-20260709-DEV-ATC-001\central-pms-rerun-3-build.log` |
| Runtime stdout capture | `D:\ExitPass-UAT-Evidence\CPS-POS-UAT-20260709-DEV-ATC-001\central-pms-rerun-3-stdout.log` |
| Runtime stderr capture | `D:\ExitPass-UAT-Evidence\CPS-POS-UAT-20260709-DEV-ATC-001\central-pms-rerun-3-stderr.log` |
...
| Readiness observation | TCP listener opened on approved port before the single `/run` request |
| Stopped after run | Yes |
| Build evidence | `D:\ExitPass-UAT-Evidence\CPS-POS-UAT-20260709-DEV-ATC-001\pos-server-rerun-3-build.log` |
| Runtime stdout capture | `D:\ExitPass-UAT-Evidence\CPS-POS-UAT-20260709-DEV-ATC-001\pos-server-rerun-3-stdout.log` |
| Runtime stderr capture | `D:\ExitPass-UAT-Evidence\CPS-POS-UAT-20260709-DEV-ATC-001\pos-server-rerun-3-stderr.log` |

The Central PMS controlled response reported `posServerCallAttempted=false`, so POS Server was not called for fiscal document creation during rerun-3.
...
The fixture preparation failure remains the active blocker. The response intentionally redacted underlying exception details.
...
| --- | --- |
| `controlled-posserver-fiscal-uat-CPS-POS-UAT-20260709-DEV-ATC-001-rerun-3-request.json` | Approved request body submitted to Central PMS |
| `controlled-posserver-fiscal-uat-CPS-POS-UAT-20260709-DEV-ATC-001-rerun-3-run-response.json` | Central PMS controlled response |
| `controlled-posserver-fiscal-uat-CPS-POS-UAT-20260709-DEV-ATC-001-rerun-3-execution-summary.json` | Execution summary |
| `controlled-posserver-fiscal-uat-CPS-POS-UAT-20260709-DEV-ATC-001-rerun-3-hash.txt` | SHA-256 checksums |
| `central-pms-rerun-3-build.log` | Central PMS build output |
| `pos-server-rerun-3-build.log` | POS Server build output |
| `central-pms-rerun-3-stdout.log` | Central PMS runtime stdout capture |
| `central-pms-rerun-3-stderr.log` | Central PMS runtime stderr capture |
| `pos-server-rerun-3-stdout.log` | POS Server runtime stdout capture |
| `pos-server-rerun-3-stderr.log` | POS Server runtime stderr capture |

## 10. SHA-256 Checksums

```text
A485636659EA9BBA8497BA42CF4A6422EE05AB367EF04C892D8C8601DC10DAD0  controlled-posserver-fiscal-uat-CPS-POS-UAT-20260709-DEV-ATC-001-rerun-3-request.json
20741A5D0F873750E602E3F16BD369A75C957FB4F48F4C5A0DBBB5DD39F87D1F  controlled-posserver-fiscal-uat-CPS-POS-UAT-20260709-DEV-ATC-001-rerun-3-run-response.json
6F6C13D383723C4B7987FF17DCD1A83B903E7AFFF32D642826D1A1DAEB162C38  controlled-posserver-fiscal-uat-CPS-POS-UAT-20260709-DEV-ATC-001-rerun-3-execution-summary.json
FF5F54EE4BFC5099B4AEA2FD7BF69BAC0AA7FBBAFAC86E4FEF7988559AF0253E  central-pms-rerun-3-build.log
C93B58409EC31B19AB83C7A80DF36E56E9FA4617A53DCFD4973D83C1F774DB6F  pos-server-rerun-3-build.log
EF4B22C4EC59D514AC59AC389A6B22FDE5A265DD2CC3C4BBD3FFA58ACCAD21D7  central-pms-rerun-3-stdout.log
E3B0C44298FC1C149AFBF4C8996FB92427AE41E4649B934CA495991B7852B855  central-pms-rerun-3-stderr.log
94BF1ADF60D3596D588D05B4523A5A61652886B79E5A3AE19BD0BE187BCEEA94  pos-server-rerun-3-stdout.log
E3B0C44298FC1C149AFBF4C8996FB92427AE41E4649B934CA495991B7852B855  pos-server-rerun-3-stderr.log
```
...
No additional runtime endpoint was called for side-effect checking after the single approved `/run` response. The response itself shows the run stopped before fiscal reference preparation, diagnostic invocation, and POS Server fiscal document creation.
...
The effective execution blocker remains controlled UAT fixture preparation in Central PMS. Because the approved rerun-3 request was already sent once, further execution requires a new approval or follow-up diagnostic authorization.

## 13. Final Result

Final result: `failed`

Reason: Central PMS returned HTTP `409` with status `controlled_uat_fixture_prepare_failed` before fiscal issuance reference preparation, before diagnostic invocation, and before POS Server call attempt.
