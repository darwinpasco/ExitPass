# ExitPass Central PMS + POS Server Real Fiscal Void Conflict Runtime Result v1.0

## Purpose

Record the controlled non-production conflict/fail-closed smoke for the Central PMS real POS Server fiscal document void integration.

## Execution Timestamp

- Executed at: 2026-07-09T16:56:29.2095429Z
- Local evidence timestamp: 2026-07-10 00:56 Asia/Manila
...
- Approved fixture existed before conflict: passed.
- Approved fixture posture before conflict: `voided`, `void_status=recorded`.
- Prior void idempotency key existed for the approved document: passed.
- Fiscal sequence value before conflict: `2`.
- Document count for `SI-00000002-UAT` before conflict: `1`.

## Conflict Fact

- Scenario flag: `real_void_conflict`
- Same idempotency key as first real void and replay.
- Same approved document id, reason code, requested-by ref, source system ref, business day, profile id, fiscal issuance reference id, POS fiscal document id, and correlation id.
- Controlled semantic difference: POS Server `reasonText` changed to `CONTROLLED_UAT_REAL_VOID_CONFLICT_TEXT`.

## Result

- HTTP result: `409`
- Central PMS result status: `pos_server_void_conflict`
- POS Server result classification: `conflict`
- POS Server error code surfaced by Central PMS: `fiscal_document_void_idempotency_conflict`
- POS Server void/cancellation status after conflict: `recorded`
- Fiscal document status posture after conflict: `voided`
- Fiscal sequence value before conflict: `2`
- Fiscal sequence value after conflict: `2`
...
- Final successful conflict evidence prefix: `controlled-posserver-fiscal-uat-CPS-POS-UAT-20260709-DEV-ATC-001-real-void-conflict-runtime-final`
- SHA-256 manifest: `controlled-posserver-fiscal-uat-CPS-POS-UAT-20260709-DEV-ATC-001-real-void-conflict-runtime-sha256.json`
