...
| MP-MDR-008 | MDR-FR-028 to 031 | Statutory benefit, evidence access, operator, and supervisor review visibility | Head Office Statutory Benefit Review facade and UI implement server-paginated WebPay/APT request visibility, separately authorized safe evidence metadata, terminal approve/reject actions, explicit GLOBAL/Site/Site Group authority, PHP-only presentation, and Central PMS canonical decision delegation. Contract: `management-platform-statutory-benefit-review:v1`. Feature-pinned runtime acceptance `MP-BENEFIT-ACCEPT-20260824T030244Z-B0B4564A` proves actual H-006/TOTP sessions, canonical WebPay/APT intake, cross-Site visibility, approval, reasoned rejection, terminal conflict, CSRF, denied permission, responsive Chromium, logout, storage, and no-external-request boundaries. | `IMPLEMENTED\_AND\_ACCEPTED` | Broader statutory/coupon management reporting, raw evidence download, workflow reversal, and client-side entitlement application remain deferred. |
