...
| Management Dashboard and Reporting | `PARTIAL` | Central PMS phase-1 catalog and scoped operational overview, runtime-accepted Payment and Reconciliation backend/UI foundation, and implemented-not-accepted fiscal exception backend foundation | Fiscal exception frontend/runtime acceptance, management activity reporting, exports, schedules, and wider dashboard acceptance remain |
...
| MP-MDR-004 | MDR-FR-013 to 016 | Degraded-watch, degraded-active, Continuity Terminal, manual release, and fiscal exception visibility | Fiscal exception backend exposes aggregate failed, conflict, outcome-unavailable, and lifecycle states from Central PMS coordination references; continuity, deadline-based overdue detection, and frontend remain absent | `PARTIAL` | Implement fiscal frontend/runtime acceptance and separately approve continuity exception sources |
...
| MP-MDR-006 | MDR-FR-022 to 024 | Sales Invoice issuance, fiscal status, and authorized report-reference visibility | `GET /v1/management-platform/dashboard/fiscal-exception-summary`; contract `management-platform-fiscal-exception-reporting:v1`; `sales-invoice-report.view` / `ManagementPlatformFiscalExceptionSummaryRead`; typed feature `ManagementPlatform:DashboardReporting:FiscalExceptions:Enabled`; Central PMS `core.fiscal_issuance_references` and linked confirmations; three supported aggregate exception categories with Site/Site Group, period, privacy, audit, hosted-session, and PostgreSQL proofs | `IMPLEMENTED\_NOT\_ACCEPTED` | Add Management Platform frontend and merged-source runtime acceptance; printing, delivery, overdue, retry-exhaustion, adjustment, void, reprint, and BIR report facts remain unavailable |
...
| MP-MDR-009 | MDR-FR-032 to 037 | Export control, filters, freshness, source labels, access/export audit, privacy | Runtime-accepted Payment and Reconciliation UI and the fiscal backend require explicit SITE/SITE_GROUP scope and UTC half-open bounds up to 31 days. Both expose source/freshness/warnings and privacy-safe success, denial, invalid-query, disabled-feature, and source-failure behavior; fiscal frontend and export remain absent | `PARTIAL` | Add fiscal frontend and separately approve controlled export |
...
| MP-MDR-011 | MDR-AC-001 to 020 | Satisfy the approved dashboard acceptance criteria | Payment and Reconciliation backend/UI foundation is runtime accepted at Central PMS `89ddf5db462fab1e1694d2cf8320d37d5f6cbf5d` and Management Platform `eed4aeef1b16116e847de701df843b14fb416516`; fiscal exception backend is implemented but awaits frontend and runtime acceptance; broader operational, activity, export, and schedule acceptance remains incomplete | `PARTIAL` | Complete and accept the remaining approved dashboard/reporting slices |
