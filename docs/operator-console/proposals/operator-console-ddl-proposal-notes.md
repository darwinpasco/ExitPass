...
- Access evaluation timing is locked separately from persistence: evaluate at workflow start and again before every controlled action. Controlled actions include starting statutory validation, submitting statutory approval/rejection, capturing evidence, viewing stored evidence, requesting shift takeover, approving/rejecting shift takeover, revoking shift access, supervisor override, and report export.
- Access evaluation responses are authoritative only at the time of the evaluated action and must not be cached or reused as long-lived permission grants because user, role, device, site, shift, takeover, or revocation state can change during a workflow.
- `operator_console.operator_access_evaluations` persists only denied and controlled-action evaluations for MVP, with correlation, user, device, shift, site, and target fields. It does not persist every page load, tab switch, harmless read, read-only module browsing, or navigation event.
