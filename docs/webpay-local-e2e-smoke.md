...
| Payment Orchestrator API | `http://localhost:8082` | `http://localhost:8082` |
...
$env:ConnectionStrings__MainDatabase = "<approved local connection string>"
...
```powershell
Invoke-WebRequest -Uri "http://localhost:8082/swagger/v1/swagger.json" -UseBasicParsing
...
Find payment attempts created for the smoke:
...
If the local smoke only reaches provider handoff, the confirmation and exit authorization counts should remain `0`.

## What This Smoke Proves

- WebPay can call Payment Orchestrator locally.
- Payment Orchestrator can call Central PMS locally.
- Central PMS can provide parking-session/tariff context for payment initiation.
- WebPay can initiate a PaymentAttempt/provider handoff.
...
It validates `Integrations:CentralPms:BaseUrl`, `Payments:Providers:PayMongo:SecretKey`, `Payments:Providers:PayMongo:PublicKey`, and `Payments:Providers:PayMongo:BaseUrl`. Use local placeholders only when the provider base URL points to a local stub.
